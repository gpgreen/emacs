...
(add-hook 'find-file-hooks 'agulbra-clean-out-spaces)
...
(defun agulbra-switch-cpp-h ()
  "Switch to the corresponding .cpp or .h file."
  (interactive)
  (let ((n (buffer-name)))
    (cond ((string-match "\\.h$" n)
	   (find-file (replace-match ".cpp" t t n)))
	  ((string-match "\\_x11.cpp$" n)
	   (find-file (replace-match ".h" t t n)))
	  ((string-match "\\_win.cpp$" n)
	   (find-file (replace-match ".h" t t n)))
	  ((string-match "\\.cpp$" n)
	   (find-file (replace-match ".h" t t n)))
	  (t
	   (error "%s is neither .h or .cpp" n)))))
